# ca_navigator/analysis/power_estimate.py
"""
...
combining:
  ...
  1. Compute power  — already physically grounded in the repo. Reuses the Fan,
     Weber & Barroso (ISCA 2007) utilization power model and NVIDIA Jetson Orin
     NX datasheet constants from ca_navigator/tools/orin_nx_cycle_model.py,
     aggregated across all runs in experiment_summary.csv.
  ...
  2. Propulsion power — the repo's existing propulsion_energy_j/mean_power_w
     ...
     al.) with no hover/drag/battery term (see ca_navigator/tools/energy_monitor.py).
...
All formulas/constants are cited inline. See docs/gem5_power_study.md,
docs/ca_architecture_deviations.md and docs/ARCHITECTURE.md ("Energy Model")
for the provenance of the compute-power side of this model.
"""
...
from ca_navigator.tools.orin_nx_cycle_model import (
    _TDP_W as ORIN_TDP_W,
    _IDLE_FRAC as ORIN_IDLE_FRAC,
    _N_CORES as ORIN_N_CORES,
)
...
# coefficient k2=0.04 already used in ca_navigator/simulation/physics.py
...
# ---------------------------------------------------------------------------
# CSV loading (mirrors ca_navigator/analysis/statistics_analyzer.py's
# case-insensitive column resolution so this composes with the same config).
# ---------------------------------------------------------------------------
...
def per_run_physical_metrics(df_raw: pd.DataFrame, target_distance_m: float) -> pd.DataFrame:
    ...
    ca_navigator.analysis.statistics_analyzer.run_analysis() (the pipeline's
    canonical per-strategy summary), so the physics model has one definition.
    """
    ...
    df["u_eff"] = (df["compute_power_w"] - ORIN_TDP_W * ORIN_IDLE_FRAC) / (
        (ORIN_TDP_W - ORIN_TDP_W * ORIN_IDLE_FRAC)
    )
