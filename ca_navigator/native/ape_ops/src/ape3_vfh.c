...
ape_result_t ape3_vfh_plan(const ape_params_t *p) {
    ...
    float sidestep_speed_frac_eff = ape_clampf(p->sidestep_speed_frac + 0.2f * conf, 0.0f, 0.95f);
    float base_v = ape_clampf(p->v_cmd, 0.0f, sidestep_speed_frac_eff * p->max_v);
    float vcap = ape_clampf(p->v_cap_frac + 0.2f * conf, 0.0f, 0.95f) * p->max_v;
