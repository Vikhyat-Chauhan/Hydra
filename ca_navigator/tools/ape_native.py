...
(ca_navigator/native/ape_ops/): APE1 = reactive potential-field/Bug-style
nudge, APE2 = Dynamic Window Approach, APE3 = Vector Field Histogram.
These are the REAL decision-making algorithms, not a synthetic cost
proxy — nav_algorithm_T.py's role in the APE path is to marshal sensor
data + config into a call here and unpack the result; no algorithm logic
lives in Python (see docs/ca_architecture_deviations.md for why).

Build the library once (rebuild after moving to different hardware — the
.so is architecture-specific, not portable):
    cd ca_navigator/native/ape_ops && make native
...
_LIB_PATH = (
    pathlib.Path(__file__).resolve().parents[1] / "native" / "ape_ops" / "build" / "libape_ops.so"
)
